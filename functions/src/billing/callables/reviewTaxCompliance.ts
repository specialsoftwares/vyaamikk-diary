...
import type { SubscriptionTaxComplianceDoc } from "../types";
import type { AdminAuthContext } from "../tax/adminAuth";
import {
  applyReviewTaxCompliance,
  type ReviewTaxComplianceInput,
} from "../tax/taxCompliance";

export async function applyReviewSubscriptionTaxCompliance(
  store: BillingStore,
  input: ReviewTaxComplianceInput
): Promise<SubscriptionTaxComplianceDoc> {
  return applyReviewTaxCompliance(store, input);
}

export const reviewSubscriptionTaxCompliance = onCall(
  { region: "asia-south1" },
  async (request) => {
    void request;
    if (!request.auth?.uid) {
    ...
    throw new HttpsError(
      "failed-precondition",
      "Tax-compliance review callable is not production-enabled in VYD-40."
    );
  }
);
