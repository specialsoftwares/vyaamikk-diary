/**
 * SHA-256 hex digest (sync) for email-binding keys.
 * Used client-side and mirrored server-side — not for OTP digests.
 */
...
/** UTF-8 bytes → SHA-256 hex (lowercase). */
export function sha256Hex(message: string): string {
  ...
  // 64-bit length big-endian (high 32 always 0 for practical email lengths)
